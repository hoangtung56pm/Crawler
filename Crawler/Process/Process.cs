...
namespace Crawler.Process
{
    ...
    public class Process
    {
        ...
        private static string GetContentTeaser(string content, Record record)
        {
            ...
            HtmlNode nodes = doc.DocumentNode.SelectSingleNode(record.PartternTeaser);
        ...
        private static string GetContentBodyWithNode(string content, Record record)
        {
            ...
            try
            {
                ...
                HtmlDocument doc = new HtmlDocument();
                doc.LoadHtml(content);
                HtmlNode nodes = doc.DocumentNode.SelectSingleNode(record.PartternDetail);
        ...
        private static string GetTag(string content, Record record)
        {
            ...
            try
            {
                HtmlNode nodes = doc.DocumentNode.SelectSingleNode(record.PartternTags);

                if (nodes != null)
                {
                    string tag = nodes.InnerHtml;
                    if (tag == "") return tag;
                    doc = new HtmlDocument();
                    doc.LoadHtml(tag);

                    HtmlNodeCollection nodesList = doc.DocumentNode.SelectNodes("//a");
                    ...
                    if (nodesList != null && nodesList.Count > 0)
                    {
                        ...
                        {
                            metaTagTemp += item.InnerText + ",";
                            if (metaTagTemp.Length <= 150)
                            {
                                metaTag += item.InnerText + ",";
                            }
                        }
        ...
        private static string GetContentBody(string content, Record record)
        {
            ...
            //IEnumerable<HtmlParseError> test = doc.DocumentNode.OwnerDocument.ParseErrors;

            //if (test.Count() > 0)
            //{
            //    foreach (var item in test)
            //    {
            //        string a = item.SourceText;
            //    }
            //}
            ///html/body/form/div[5]/div/div/div/div/div/div[3]
            HtmlNode nodes = doc.DocumentNode.SelectSingleNode(record.PartternDetail);

            if (nodes != null)
            {
                if (record.PartternDetailRemove != "")
                {
                    doc = new HtmlDocument();
                    doc.LoadHtml(nodes.InnerHtml);
                    if (record.PartternDetailRemove.IndexOf(',') > -1)
                    {
                        string[] arr = record.PartternDetailRemove.Split(',');

                        if (arr.Length > 0)
                        {
                            foreach (string item in arr)
                            {
                                var nodesToRemove = doc.DocumentNode.SelectSingleNode(item);
                    ...
                    {
                        var nodesToRemove = doc.DocumentNode.SelectSingleNode(record.PartternDetailRemove);
                        if (nodesToRemove != null)
                        {
                            nodesToRemove.Remove();
                        }
        ...
        private static string OpitmisePubDate(string pubDate, string formatDate)
        {
            try
            {
                ...
                pubDate = pubDate.Replace("tháng", "/").Replace("năm", "/").Replace(" / ", "/").Replace("  ", " ").Replace("-", "/").Replace("&nbsp;", " ");//.Replace(" "," ");
                pubDate = Encoding.ASCII.GetString(Encoding.Convert(Encoding.UTF8, Encoding.GetEncoding(Encoding.ASCII.EncodingName, new EncoderReplacementFallback(" "), new DecoderExceptionFallback()), Encoding.UTF8.GetBytes(pubDate)));
                string[] arr = pubDate.Trim().Split(' ');

                if (arr.Count() > 1)
                {
                    if (arr[0].ToString().Length > 3 && arr[0].IndexOf(":") > -1)
                    {
                ...
                if (pubDate != "")
                {
                    ...
                    if (arr[0].ToString().Length > 3 && arr[0].IndexOf(":") > -1)
                    {
                        ...
                        if (formatDate == "yyyy/MM/dd")
                        {
                            ...
                            return arrDate[2] + "/" + arrDate[1] + "/" + year;
                        }
                        if (formatDate == "MM/dd/yyyy")
                        {
                            return arrDate[1] + "/" + arrDate[0] + "/" + year + " " + arr[0];
                        }
                        return arrDate[0] + "/" + arrDate[1] + "/" + year + " " + arr[0];
                    }
                    else
                    {
                        arrDate = arr[0].ToString().Split('/');
                        string year = "";
                        if (arrDate.Length > 2)
                        {
                            year = arrDate[2];
                        ...
                        if (formatDate == "yyyy/MM/dd")
                        {
                            ...
                            return arrDate[2] + "/" + arrDate[1] + "/" + year + " " + time;
                        }
                        if (formatDate == "MM/dd/yyyy")
                        {
                            return arrDate[1] + "/" + arrDate[0] + "/" + year + " " + time;
                        }
                        return arrDate[0] + "/" + arrDate[1] + "/" + year + " " + time;
                    }
            ...
            catch (Exception)
            {
                try
                {
                    Match match = Regex.Match(pubDate, "(?<Day>\\d{1,2})/(?<Month>\\d{1,2})/(?<Year>(?:\\d{4}|\\d{2}))", RegexOptions.IgnoreCase | RegexOptions.Singleline);
                    pubDate = match.Groups["Day"].Value + "/" + match.Groups["Month"].Value + "/" + match.Groups["Year"].Value;
                    return pubDate;
                }
                catch (Exception)
                {
        ...
        private static string GetPubDateByParttern(string content, Record record)
        {
            ...
            try
            {
                ...
                doc.LoadHtml(content);
                HtmlNode nodes;
                if (record.PartternPubDate.IndexOf('_') > 0)
                {
                    string[] arr = record.PartternPubDate.Split('_');
                    nodes = doc.DocumentNode.SelectSingleNode(arr[0].ToString());
                    if (nodes != null)
                    {
                        pubDate = nodes.GetAttributeValue(arr[1].ToString(), "");
                    }
                }
                else
                {
                    nodes = doc.DocumentNode.SelectSingleNode(record.PartternPubDate);
                    pubDate = nodes.InnerText;
                }
