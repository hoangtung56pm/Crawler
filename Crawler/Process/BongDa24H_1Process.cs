...
{
    public class BongDa24H_1Process
    {
            ...
            try
            {
                ...
                if (xdoc != null)
                {
                    var res = from item in xdoc.Descendants(xmlns + "div")
                              where item.Attribute("class") != null && item.Attribute("class").Value == "DivCateSALEHNNPII"
                              select new
                                         {
                                             Link = item.Element(xmlns + "a").Attribute("href").Value,
                                             Image = "",
                                             Title = item.Element(xmlns + "a").Element(xmlns + "span").Value,
                                             Hour = "00:00",
                                             Date = "",
                                             Desc = ""
                                         };
