            ...
            try
            {
                ...
                if (xdoc != null)
                {
                    ...
                    foreach (var node in res)
                    {
                        ...
                        //                   item.Attribute("class").Value == "detail_title"
                        //               select new
                        //                          {
                        //                              Title = item.Value,
                        //                          };
                        ...
                        var resDate = from item in xdoc.Descendants(xmlns + "div")
                                      where
                                          item.Attribute("class") != null &&
                                          item.Attribute("class").Value == "detail_date"
                                      select new
                                                 {
                                                     Date = item.Value,
                                                 };
                        ...
                        var resTeaser = from item in xdoc.Descendants(xmlns + "div")
                                        where
                                            item.Attribute("class") != null &&
                                            item.Attribute("class").Value == "spshare"
                                        select new
                                                   {
                                                       Teaser = item.Element(xmlns + "h2").Value,
                                                   };
                        ...
                        var resBody = from item in xdoc.Descendants(xmlns + "div")
                                      where
                                          item.Attribute("class") != null &&
                                          item.Attribute("class").Value == "detail_content"
                                      select new
                                                 {
                                                     Body = item.Value,
                                                 };
                        ...
                        string body = resBody.ElementAt(0).Body;
